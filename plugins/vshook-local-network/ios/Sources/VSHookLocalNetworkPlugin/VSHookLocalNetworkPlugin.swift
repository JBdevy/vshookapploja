...
public class VSHookLocalNetworkPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    public let pluginMethods: [CAPPluginMethod] = [
        ...
        CAPPluginMethod(name: "discoverApplePeers", returnType: CAPPluginReturnPromise),
        CAPPluginMethod(name: "connectApplePeer", returnType: CAPPluginReturnPromise),
        CAPPluginMethod(name: "stopApplePeerBridge", returnType: CAPPluginReturnPromise)
    ]

    private let applePeerBridge = VSHookApplePeerBridge()
    ...
    @objc func discoverApplePeers(_ call: CAPPluginCall) {
        let timeoutMs = max(1200, min(12000, call.getInt("timeoutMs") ?? 3500))
        applePeerBridge.discover(timeout: TimeInterval(timeoutMs) / 1000.0) { result in
            switch result {
            case .success(let peers):
                call.resolve(["peers": peers.map { ["id": $0.id, "name": $0.name] }])
    ...
    @objc func connectApplePeer(_ call: CAPPluginCall) {
        guard let peerId = call.getString("peerId"), !peerId.isEmpty else {
            call.reject("Selecione um Mac para conectar.")
            return
        }
        applePeerBridge.connect(peerId: peerId) { result in
            switch result {
            case .success(let urls):
                call.resolve([
                    "directorUrl": urls.directorUrl,
                    "musiciansUrl": urls.musiciansUrl,
                    "peerName": urls.peerName,
    ...
    @objc func stopApplePeerBridge(_ call: CAPPluginCall) {
        applePeerBridge.stop()
...
private struct VSHookApplePeer: Equatable {
    let id: String
    let name: String
}

private struct VSHookApplePeerURLs {
...
private final class VSHookApplePeerBridge {
