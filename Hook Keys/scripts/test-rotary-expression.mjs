...
test('Modulation toggle persists; CC 1 updates the correct module only at Slow/Fast crossings; mapped buttons use press edges', () => {
  ...
  assert.equal(modules[4].settings.rotary.speed, 'fast', 'enabling uses the last selected MIDI Mod value');
  assert.equal(JSON.parse(JSON.stringify(modules[4].settings)).rotary.modulationEnabled, true);
  screen.handleMidiControlChange({ controller: 1, value: 0, inputId: 'other-midi' });
  assert.equal(modules[4].settings.rotary.speed, 'fast', 'different assigned device cannot alter Rotary');
  ...
  assert.equal(modules[4].settings.rotary.speed, 'slow', 'virtual keyboard Mod broadcast controls Rotary');
  screen.toggleRotaryModulation();
