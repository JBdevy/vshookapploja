...
export function createTranceGateMarkup(value: unknown): string {
  ...
  return `<section class="pattern-editor sequencer-editor trance-gate-editor" data-trance-gate-editor>
    <header class="pattern-editor__header"><div><span>Módulo 07 · Sync BPM</span><strong>Trance Gate</strong></div><output>${settings.length} passos · ${settings.division}</output></header>
    ...
    <div class="sequencer-steps" role="group" aria-label="Passos do Trance Gate">${settings.steps.map((enabled, index) =>
      `<button type="button" class="sequencer-step ${enabled ? 'is-enabled' : 'is-disabled'}${index >= settings.length ? ' is-outside' : ''}" data-trance-gate-step="${index}" aria-label="Passo ${index + 1}" aria-pressed="${enabled}" ${index >= settings.length ? 'disabled' : ''}><span>${String(index + 1).padStart(2, '0')}</span><strong>${enabled ? 'ON' : 'OFF'}</strong><i></i></button>`).join('')}</div>
